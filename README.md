...
1： memberNo如果传入随机数，订单号，每次订单都不一样，需要每次都绑定银行卡，用户体验不好，会影响成功率。 <br>

2： memberNo如果写死一个值，会员A，B，C，D。。。只会绑定一次银行卡，但彼此都能看见自己的银行卡，会影响用户体验。。会害怕。。  <br>

3： memberNo需要传会员ID或代表会员ID唯一的信息（如：加密后的会员ID，混淆的会员ID)，只会绑定一次银行卡，而且各自订单只能看见自己的银行卡，体验好，成功率高。<br>
